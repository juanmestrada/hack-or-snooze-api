...
class Story {
  ...
  getHostName() {
    ...
    const hostName = this.url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "").split('/')[0];
...
class StoryList {
  ...
  async addStory(user, newStory) {
    try {
      ...
      // add new story to storlist instance and currentUser.ownStories array
  ...
  async removeStory(storyId) {
    ...
    try {
      ...
      // remove story from StoryList instance
      this.stories = this.stories.filter(s => s.storyId !== storyId);
  
      // remove story from currentUser's ownStories && favorites
...
class User {
  ...
  checkIfOwnStory(story){
    return this.ownStories.some(e => (e.storyId === story.storyId));
  }
  async toggleFavorite(type, story) {
    const method = type === "post" ? "POST" : "DELETE";
